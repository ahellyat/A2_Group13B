...
function drawStartScreen() {
  ...
  ellipse(180, 80, 160, 60);
  ellipse(260, 65, 120, 50);
  ellipse(130, 72, 90, 45);
  ellipse(560, 95, 140, 52);
  ellipse(640, 80, 110, 46);
  ellipse(950, 70, 155, 58);
  ellipse(1040, 58, 130, 50);
...
function drawMenuBooth() {
  ...
  for (let i = 0; i * bsw < armLen + bsw; i++) {
    ...
    noStroke();
    rect(armBaseX + 8 + i * bsw, armBaseY - 8, bsw, 9);
  }
  drawingContext.restore();
  stroke(80, 80, 90);
  strokeWeight(1);
...
function drawMenuCar(carX, rY) {
  ...
  // Headlights
  fill(lerpColor(carCol, color(30), 0.35));
  stroke(0);
  strokeWeight(1);
  rect(bR - 1, bTop + 8, 7, 20, 2);
  fill(255, 255, 190);
  noStroke();
  rect(bR, bTop + 10, 4, 8, 1);
  fill(255, 235, 140);
  rect(bR, bTop + 20, 4, 4, 1);
...
function drawInstructionsPanel() {
  ...
  textAlign(LEFT, TOP);
  text(
    "You are a parking booth attendant. Animals arrive one by one and you must",
    tx,
    ty,
  );
  ty += lineH;
  text(
    "process each guest correctly before opening the gate to let them through.",
    tx,
    ty,
  );
  ty += lineH + 14;
  ...
  textAlign(LEFT, TOP);
  text(
    "Your first two guests are training guests. For each one, click the 5 booth",
    tx,
    ty,
  );
  ty += lineH;
  text(
    "controls in any order you like — this sequence becomes your personal ritual",
    tx,
    ty,
  );
  ty += lineH;
  text(
    "for that badge type. There is no wrong answer during training!",
    tx,
    ty,
  );
  ty += lineH + 14;
  ...
  textAlign(LEFT, TOP);
  text(
    "From Guest #3 onward, cars show a square badge, triangle badge, or no badge.",
    tx,
    ty,
  );
  ty += lineH;
  text(
    "Recreate the matching ritual — same controls, same order — then open the gate.",
    tx,
    ty,
  );
  ty += lineH;
  text(
    "Guests with no badge are free-choice: click anything you like for them.",
    tx,
    ty,
  );
  ty += lineH + 14;
  ...
  textAlign(LEFT, TOP);
  text(
    "Every wrong submission worsens the weather and fogs up your control panel.",
    tx,
    ty,
  );
  ty += lineH;
  text(
    "Four mistakes ends your shift. Get it right to clear the skies again!",
    tx,
    ty,
  );
...
function drawAttendantBadge() {
  let cx = width / 2;
  // Badge sits centred above the title sign
  let badgeCY = height / 2 - 258;
  let r = 52; // badge radius

  // ── Outer badge ring (gold) ──
  fill(200, 165, 60);
  stroke(140, 105, 20);
  strokeWeight(2);
  ellipse(cx, badgeCY, r * 2 + 14, r * 2 + 14);

  // Decorative notched ring inside gold border
  noFill();
  stroke(140, 105, 20);
  strokeWeight(1);
  let notches = 32;
  for (let i = 0; i < notches; i++) {
    let a = (i / notches) * TWO_PI;
    let r1 = r + 3,
      r2 = r + 6;
    line(
      cx + cos(a) * r1,
  ...
  // ── Badge face (navy circle) ──
  fill(13, 67, 102);
  stroke(8, 44, 70);
  strokeWeight(2);
  ellipse(cx, badgeCY, r * 2, r * 2);

  // ── Tiny stars left & right on badge ──
  fill(200, 165, 60);
  noStroke();
  textAlign(CENTER, CENTER);
  textSize(9);
  text("★", cx - r * 0.62, badgeCY + r * 0.72);
  text("★", cx + r * 0.62, badgeCY + r * 0.72);

  // ── "PAWS PARKING AUTHORITY" arc text along top of badge ──
  fill(200, 165, 60);
  noStroke();
  textAlign(CENTER, CENTER);
  textSize(7.5);
  let arcLabel = "PAWS  PARKING  AUTHORITY";
  let arcR = r - 9;
  let arcSpan = PI * 0.78;
  let arcStart = -HALF_PI - arcSpan / 2;
  for (let i = 0; i < arcLabel.length; i++) {
    let t = i / (arcLabel.length - 1);
    let a = arcStart + t * arcSpan;
    push();
    translate(cx + cos(a) * arcR, badgeCY + sin(a) * arcR);
    rotate(a + HALF_PI);
    text(arcLabel[i], 0, 0);
    pop();
  }

  // ── Attendant figure inside the badge ──
  let headR = r * 0.26;
  let headY = badgeCY - r * 0.12;
  ...
  drawingContext.save();
  drawingContext.beginPath();
  drawingContext.arc(cx, badgeCY, r - 2, 0, TWO_PI);
  drawingContext.clip();
  ...
  fill(200, 165, 60);
  noStroke();
  rect(cx + headR * 0.15, headY + headR * 1.28, headR * 0.75, headR * 0.4, 2);
  ...
  fill(8, 44, 90);
  noStroke();
  rect(cx - headR * 1.28, headY - headR * 0.72, headR * 2.56, headR * 0.28, 2);

  // Gold cap band
  fill(200, 165, 60);
  noStroke();
  rect(cx - headR * 0.92, headY - headR * 0.73, headR * 1.84, headR * 0.18);

  // Eyes
  fill(40, 25, 10);
  noStroke();
  ellipse(cx - headR * 0.3, headY - headR * 0.08, headR * 0.22, headR * 0.22);
  ellipse(cx + headR * 0.3, headY - headR * 0.08, headR * 0.22, headR * 0.22);

  // Eye glints
  fill(255);
  noStroke();
  ellipse(cx - headR * 0.26, headY - headR * 0.13, headR * 0.09, headR * 0.09);
  ellipse(cx + headR * 0.34, headY - headR * 0.13, headR * 0.09, headR * 0.09);

  // Smile
  noFill();
  stroke(140, 80, 50);
  strokeWeight(1.2);
  arc(cx, headY + headR * 0.15, headR * 0.55, headR * 0.38, 0, PI);
