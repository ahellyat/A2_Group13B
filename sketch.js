...
function drawCloudy() {
  noStroke();
  fill(160, 168, 182);
  ellipse(200, 60, 200, 85); ellipse(370, 38, 240, 95);
  ellipse(540, 68, 180, 78); ellipse(720, 48, 210, 90);
  ellipse(910, 62, 185, 82); ellipse(1110, 42, 220, 88);
  fill(140, 148, 162);
  ellipse(140, 82, 160, 68); ellipse(440, 58, 195, 75);
  ellipse(640, 52, 200, 80); ellipse(860, 68, 170, 70);
  ellipse(1060, 58, 210, 76);
}

function drawSunny() {
  let sunX = width - 120, sunY = 80, sunR = 50;
...
function drawRain(intensity) {
  ...
  ellipse(200, 60, 180, 80); ellipse(350, 40, 220, 90);
  ellipse(500, 70, 160, 70); ellipse(700, 50, 200, 85);
  ellipse(900, 65, 170, 75); ellipse(1100, 45, 210, 80);
  fill(80, 90, 110, cloudAlpha);
  ellipse(150, 80, 140, 60); ellipse(420, 60, 180, 70);
  ellipse(620, 55, 190, 75); ellipse(850, 70, 160, 65);
  ...
  for (let i = 0; i < activeCount; i++) {
    ...
    if (d.y > height) { d.y = random(-100, -10); d.x = random(width); }
  }
}

function drawThunder() {
  if (lightningFlash) {
    fill(255, 255, 200, 80); noStroke();
    rect(0, 0, width, height);
    lightningFlash = false;
  }
  for (let i = thunderbolts.length - 1; i >= 0; i--) {
    let tb = thunderbolts[i];
    stroke(255, 255, 100, tb.alpha); strokeWeight(3);
    let x = tb.x, y = tb.startY;
    beginShape();
    for (let seg of tb.segments) { vertex(x + seg.dx, y + seg.dy); y += seg.dy; }
    endShape();
    tb.alpha -= 8;
  ...
  if (lightningTimer > 60 && random() < 0.04) {
    lightningTimer = 0; lightningFlash = true;
    let segs = [], totalY = 0;
    while (totalY < 200) {
      let dy = random(20, 50);
    ...
    thunderbolts.push({ x: random(100, width-100), startY: random(60, 120), segments: segs, alpha: 255 });
  }
}
...
function mousePressed() {
  ...
  if (mouseX >= submitX && mouseX <= submitX + submitSize &&
      mouseY >= submitY && mouseY <= submitY + submitHeight) {
    if (clickHistory.length === 0) { triedToOpenGateWithoutSubmission = true; return; }
    ...
    if (submissions.length === 1) {
      ...
      if (guestShape === "none") {
        ...
        let patternToMatch = guestShape === "triangle"
          ? submissions[1] : submissions[0];
        isMatch = patternToMatch.length === clickHistory.length &&
...
function drawGameOver() {
  ...
  // Card body
  fill(22, 22, 28); stroke(180, 20, 20); strokeWeight(3);
  rect(cX, cY, cW, cH, 12);

  // Red top bar
  fill(160, 20, 20); noStroke();
  ...
  // Icon — rain cloud
  let ix = width / 2, iy = cY + 100;
  fill(100, 110, 130); noStroke();
  ...
  // Rain drops
  stroke(150, 180, 220, 200); strokeWeight(2);
  for (let [dx, dy] of [[-28, 18], [-12, 24], [4, 18], [20, 24], [36, 18]]) {
  ...
  text("Too many mistakes — the guests have lost patience.", width / 2, cY + 188);

  // Sub-message
  fill(140, 140, 160); textSize(13);
  text("Refresh the page to try again.", width / 2, cY + 218);

  // Bottom rule
  stroke(80, 20, 20); strokeWeight(1);
  line(cX + 30, cY + cH - 28, cX + cW - 30, cY + cH - 28);
  noStroke(); fill(100, 20, 20); textSize(11);
...
function drawFeedbackShape() {
  let alpha = feedbackShapeTimer > 60
    ? 255
    : map(feedbackShapeTimer, 0, 60, 0, 255); // fade out in last second
  ...
  if (feedbackShapeType === "square") {
    ...
    let hh = sz * 0.866;
    triangle(cx, cy - hh * 0.8, cx - sz * 0.8, cy + hh * 0.8, cx + sz * 0.8, cy + hh * 0.8);
  }

  // Shape
  if (feedbackShapeType === "square") {
    fill(40, 120, 220, alpha);
    stroke(255, 255, 255, alpha * 0.6); strokeWeight(2);
